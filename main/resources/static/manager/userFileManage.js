...
function system_noti_page(page) {
	...
	AjaxMethod.ajax('upDownLoadController/getUserFileList', {
		'page' : page,
        ...
        if (msg_arr && msg_arr.length > 0) {
            ...
            for (var i in msg_arr) {
            	var msg = msg_arr[i];
            	noti_html += join_system_noti(msg_arr[i].fileId, msg_arr[i].fileName, msg_arr[i].sender, msg_arr[i].receiver, msg_arr[i].levels, msg_arr[i].sendTime);
            }	
...
//拼接日志消息html
function join_system_noti(fileId, fileName, sender, receiver, levels, sendTime) {
	var receiverName = "";
	if (getNameById(receiver)) {
		receiverName = getNameById(receiver);
	} else {
		receiverName = getGroupNameById(receiver);
	}
	var receiverLevel = (changeLevelIdToName(getLevels(receiver)) == null ? changeLevelIdToName(getGroupNameById(receiver, "1")) : changeLevelIdToName(getLevels(receiver)));
	return "<tr>"
	  ...
	  +  "</td>" 
	  +  "<td class='center'>" + (receiverName == null ? "无" : receiverName) + "(" + (receiverLevel == null ? "无" : receiverLevel) + ")"
	  +  "</td>" 
	  +  "<td class='center'>" + changeLevelIdToName(levels)
	  +  "</td>" 
	  +  "<td class='center'>" + changeDate(sendTime)
	  +  "</td>" 
	  +  "<td class='center'><input type='button' value='下载' onclick='download(\""+fileId+"\", \""+fileName+"\", \""+levels+"\")' /></td>" 
	  +  "</tr>"
}

//文件下载
function download(fileId, fileName, levels) {
	if (levels == "1") {
		AjaxMethod.ajax('homeController/queryFile', {'fileId' : fileId, 'uploadType' : 'user'}).then(function (result){
	        if (result){
